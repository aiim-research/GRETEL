...
class Molecule(BaseEnvironment):
    ...
    def __init__(
        ...
        super(Molecule, self).__init__(target_fn=target_fn, max_steps=max_steps)
    ...
    def set_instance(self, new_instance):
        ...
        self.atom_types = np.unique(
            [
                x.GetSymbol()
                for x in mol_from_smiles(self._init_instance.smiles).GetAtoms()
            ]
        )
    ...
    def _atom_additions(
        self, state, atom_types, atom_valences, atoms_with_free_valence
    ):
        bond_order = {
                ...
                for element in atom_types:
                    if atom_valences[element] >= i:
                        ...
                        if sanitization_result:
                            continue
                        data_instance = MolecularDataInstance(
                            self._state.id + self.action_counter
                        )
