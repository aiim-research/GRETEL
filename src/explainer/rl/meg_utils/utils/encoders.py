...
class MorganBitFingerprintActionEncoder(ActionEncoderAB):
    def __init__(self, fp_len=1024, fp_rad=2, context: Context = None):
        ...
        self.context.logger.info(
            f"Initialized MorganBitFingerprintActionEncoder with name: {self._name}, length: {fp_len}, radius: {fp_rad}"
        )
...
class MorganCountFingerprintActionEncoder(ActionEncoderAB):
    def __init__(self, fp_length=1024, fp_radius=2, context: Context = None):
        ...
        self.context.logger.info(
            f"Initialized MorganCountFingerprintActionEncoder with name: {self._name}, length: {fp_length}, radius: {fp_radius}"
        )

    def encode(self, action: DataInstance) -> np.array:
        assert isinstance(action, MolecularInstance)
        self.context.logger.info(
            f"Encoding action with molecule ID: {action.id}, SMILES: {action.smiles} and shape: {action.data.shape}"
        )
        fp = AllChem.GetHashedMorganFingerprint(
            action.molecule, self.fp_radius, self.fp_length, bitInfo=None
        )
        self.context.logger.info("Morgan fingerprint calculated.")
        encoded = Fingerprint(fp, self.fp_length)
        self.context.logger.info("Fingerprint object created.")
...
class RDKitFingerprintActionEncoder(ActionEncoderAB):
    def __init__(self, fp_length=1024, fp_radius=2, context: Context = None):
        ...
        self.fp_radius = fp_radius
        self.context.logger.info(
            f"Initialized RDKitFingerprintActionEncoder with name: {self._name}, length: {fp_length}, radius: {fp_radius}"
        )
    ...
    def encode(self, action: DataInstance) -> np.array:
        ...
        self.context.logger.info("RDKFingerprint calculated.")
