...
1a -> 1b) it promotes the overshoot removal to right after add and defers
swap, testing whether trying a large bulk removal early (before swap and
before single-edge removal) helps.

Building blocks (shared with var_3, which uses a different order):
