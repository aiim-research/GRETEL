...
class ResGenerator(nn.Module):
    ...
    def forward(self, node_features, edge_list, edge_attr, batch):
        ...
        if self.residuals: #TODO: Fix the booster expansion if make sense
            ...
            edge_attr_t = edge_attr.mT if edge_attr.ndim == 2 else edge_attr
            edge_weights = torch.add(edge_weights, rebuild_adj_matrix(len(node_features), edge_list, edge_attr_t, self.device))
